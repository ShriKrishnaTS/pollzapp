json.extract! @group, :id, :name, :description, :privacy, :admins_can_add_users, :share_id, :members_can_create_polls, :generic
...
json.users_count @group.group_users.active.where(blocked: false).includes(:user).where(users:{disabled: 0}).count
json.polls_count @group.polls.count
json.votes_count @group.votes.count
json.thumbnail @group.image.thumbnail.url
if @group.generic == false
...
json.requests @group.group_users.inactive do |gu|
  ...
  json.groups_count gu.user.user_groups.active.where(blocked: false).includes(:group).where.not(groups: {privacy: 2}&& {deleted: 1}).count
...
json.users @group.group_users.active.where(blocked: false).reverse do |gu|
  ...
  json.user_type gu.user_type
  json.disabled gu.user.disabled
  json.thumbnail gu.user.image.thumbnail.url
 json.groups_count gu.user.user_groups.active.where(blocked: false).includes(:group).where.not(groups: {privacy: 2}&& {deleted: 1}).count
