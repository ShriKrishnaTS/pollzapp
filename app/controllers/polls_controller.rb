class PollsController < ApplicationController
  ...
  def index
    @polls = Poll.where('deleted = 0').where(group_id: @current_user.user_groups.active.unblocked.map(&:group_id)).order(updated_at: :desc).paginate(:page => params[:page], :per_page => 25)
        ...
        unless u.id == @poll.user_id
          ...
          if !mute_till || mute_till < Time.now
            n = Notification.create user_id: u.id, group_id: @poll.group_id, poll_id: @poll.id,
                                    ...
                                    code: '110',
                                    text: @poll.user.username + ' posted a new poll ' + "#{@poll.title.upcase}" + ' in ' + @poll.group.name,
                                    parts: [
