class SearchController < ApplicationController
  ...
  def index
    ...
    if ("#{params[:q]}".length > 3)
      @contacts = @current_user.contacts.where("users.name LIKE ? OR users.name LIKE ? OR users.username LIKE ?", "#{params[:q]}%", "% #{params[:q]}%", "#{params[:q]}%").where('disabled = 0')

      @others = User.where('disabled = 0').where("username LIKE ? OR username LIKE ?", "#{params[:q]}%", "% #{params[:q]}%").where.not(id: @contacts.map(&:id))
      @people = @contacts + @others
    end
    @all_groups = GroupUser.joins(:group).where('name LIKE ? OR name LIKE ?', "#{q}%", "% #{q}%").where('groups.deleted = 0').where('(groups.privacy = 0 OR groups.privacy = 1 OR (groups.privacy = 2 AND group_users.user_id = ?)) OR (group_users.id = ? AND group_users.blocked = 0 AND group_users.active = 1)', @current_user.id, @current_user.id).uniq.order(updated_at: :desc).map(&:group).uniq
