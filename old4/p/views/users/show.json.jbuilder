...
json.groups @user.user_groups.active.where(blocked: false).includes(:group).where.not(groups: {privacy: 2}) do |gu|
  json.id gu.group_id
  json.name gu.group.name
  json.user_type gu.user_type
  json.users_count gu.group.group_users.active.where(blocked: false).includes(:group).where.not(groups: {privacy: 2}).count
