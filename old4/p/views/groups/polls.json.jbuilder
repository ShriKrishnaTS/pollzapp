json.array!(@polls) do |poll|
  ...
  if poll.ends_on > Time.now
    ...
    remaining_time = "#{days.to_i-1 > 0 ? (days.to_i-1).to_s + 'd ' : ''}#{hours.to_i-1 > 0 ? (hours.to_i-1).to_s + 'h ' : ''}#{minutes.to_i > 0 ? minutes + 'm' : ''}"
  else
    remaining_time = nil
  end
  ...
  json.user do
    json.extract! poll.user, :id, :name, :username
    json.thumbnail poll.user.image.thumbnail.url
    json.groups_count poll.user.user_groups.active.where(blocked: false).includes(:group).where.not(groups: {privacy: 2}).count
    json.votes_count poll.user.votes.count
    json.polls_count poll.user.polls.count
  end
end
