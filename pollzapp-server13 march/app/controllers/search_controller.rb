class SearchController < ApplicationController
  ...
  def index
    ...
    @contacts = @current_user.contacts.where("users.name LIKE ? OR users.name LIKE ? OR users.username LIKE ? OR users.username LIKE ?", "#{params[:q]}%", "% #{params[:q]}%", "#{params[:q]}%", "% #{params[:q]}%")
