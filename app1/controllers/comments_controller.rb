class CommentsController < ApplicationController
  ...
  def create
    ...
    if @comment.save
      ...
      @comment.poll.comments.map(&:user).uniq.each do |u|
        logger.info u
        unless u.id == @current_user.id
          ...
          text: 'New comment in poll ' + "#{@comment.poll.title.upcase!}", parts: [
              ...
              {text: 'New comment in poll ' + @comment.poll.title, link: 'pages/poll/screen', context: {poll: poll}}
